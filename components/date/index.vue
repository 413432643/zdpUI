  ...
  <div class="container" v-down>
    ...
    <div v-show="dateShow" class="date">
      ...
      <div class="date-header" v-if="type == 'date' || type == 'dates'">
        ...
        <div v-show="dayListV">
          <i class="iconfont icon-L" @click="monthPrev"></i>
        </div>
        <div>
          <!-- 十年 -->
        ...
        <div v-show="dayListV">
          <i class="iconfont icon-R" @click="monthNext"></i>
        </div>
...
const dayListClass = computed(() => (item) => {
  // 天列表样式
  return [
    currentDay == item.date && props.currentSign && item.class == "currentMonth"
      ? "currentDay"
      : "",
    defaultDay.value.indexOf(item.date) != -1 && item.class == "currentMonth"
      ? "active"
      : "",
...
const monthListClass = computed(() => (item) => {
  // 月列表样式
  return [
    defaultDay.value.indexOf(
      year.value + "-" + (item <= 9 ? "0" : "") + item
    ) != -1
      ? "select"
      : "",
  ];
...
const yearhListClass = computed(() => (item) => {
  ...
  return [
    props.type != "year" &&
    defaultDay.value.indexOf(year.value - (year.value % 10) + item - 1) != -1
      ? "select"
      : "",
    defaultDay.value == year.value - (year.value % 10) + item - 1
      ? "select"
      : "",
  ];
});
...
const dateList = computed(() => {
  ...
  let prevMonthDay =
    MonthDayList.value[month.value == 0 ? 11 : month.value - 1]; //上月天数
...
const disabledRange = (obj, day) => {
  //限制范围
  if (
    Date.parse(day) > Date.parse(props.disabledDate[0]) &&
    Date.parse(day) < Date.parse(props.disabledDate[1])
  ) {
    obj.disabled = true;
  }
};
...
const yearPrev = () => {
  if (yearListV.value || props.type == "year") {
    //上十年
    year.value -= 10;
  } else {
    year.value--; //上一年
...
const dayClick = (item) => {
  if (item.class[0] != "currentMonth" || item.disabled) return;
  //点击天
  let currentDay =
    year.value +
    "-" +
  ...
  if (props.type == "dates") {
    ...
    item.select
      ? (defaultDay.value += currentDay + " ")
      : (defaultDay.value = defaultDay.value.replace(currentDay + " ", ""));
  }
